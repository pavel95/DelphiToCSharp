...
begin
  ...
  // Send array to C# to process. Then it will came back through callback
...
begin
  // class something from C#
  something := TSomething.Create(self);

  // Set the callback method
  something.SetSomethingElseCallback(nativeInt(@SomethingElseCallback));

  // class somethingElse from C#
  somethingElse := TSomethingElse.Create(self);
  somethingElse.ShowMessage('I was created in delphi, and now i will be sent to C# dll');

  // Calling a method that will consume somethingElse object and then call a callback to delphi with this same object
